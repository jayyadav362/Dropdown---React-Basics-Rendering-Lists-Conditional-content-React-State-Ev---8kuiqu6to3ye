    ...
    <div id="main">
      ...
      >
        {states.map((state, idx) => {
          return <option value={idx}>{state.name}</option>;
        })}
      </select>
      <br />
      ...
      >
        {states[selectState].city.map((city, idx) => {
          return <option value={idx}>{city.name}</option>;
        })}
      </select>
      <br />
      <label htmlFor="landmark">Landmark: </label>
      <select
        defaultValue={selectLandmark}
        id="landmark"
        onChange={(e) => setSelectLandmark(e.target.value)}
      >
        {states[selectState].city[selectCity].landmarks.map((landmark, idx) => {
          return <option value={idx}>{landmark.name}</option>;
        })}
      </select>
      <hr />
      <div id="state-name">{states[selectState].name}</div>
      <div id="state-description">{states[selectState].description}</div>
      <div id="state-title">{states[selectState].name}</div>
      <hr />
      <div id="city-name">{states[selectState].city[selectCity].name}</div>
      <div id="city-description">
        {states[selectState].city[selectCity].description}
      </div>
      <div id="city-title">{states[selectState].city[selectCity].name}</div>
      <hr />
      <div id="landmark-name">
        {states[selectState].city[selectCity].landmarks[selectLandmark].name}
      </div>
      <div id="landmark-description">
        {
          states[selectState].city[selectCity].landmarks[selectLandmark]
            .description
        }
      </div>
